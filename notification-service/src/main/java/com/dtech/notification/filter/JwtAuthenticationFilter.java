...
import com.dtech.notification.dto.response.TokenValidResponseDTO;
import com.dtech.notification.feign.TokenFeignClient;
import com.dtech.notification.util.ExtractApiResponseUtil;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            if (authorization != null && authorization.startsWith(BEARER_PREFIX)) {
                log.info("JWT Authentication Filter Authorized - Notification");
                String token = authorization.substring(7);
                log.info("JWT Authentication Filter Token - Notification {}", token);
                log.info("JWT Authentication Filter request to token server - Notification {}", token);

                ResponseEntity<ApiResponse<Object>> validateTokenResponse = tokenFeignClient.validateToken(token);
                log.info("After response token service Notification {}", validateTokenResponse);
                Object objectApiResponse = ExtractApiResponseUtil.extractApiResponse(validateTokenResponse);
                log.info("After response token response Notification {}", objectApiResponse);
                ...
                if (!token.isBlank() && tokenValidResponseDTO.isValid()) {
                    log.info("JWT Authentication Filter Token valid - Notification {}", tokenValidResponseDTO);
                    ...
                    log.info("JWT Authentication Filter Token expired or invalid - Notification {}", token);
